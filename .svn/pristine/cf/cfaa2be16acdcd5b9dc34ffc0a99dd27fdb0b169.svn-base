...
void drawTexture(int texNum, Rect pos, Rect texcoord, double z, double a, double rotationAngle) {
	glEnable(GL_TEXTURE_2D);
	int windowID = glutGetWindow();
	Texture& texture = globalTexture[texNum];
	glBindTexture(GL_TEXTURE_2D, texture.win2id[windowID]);
	
	float texleft = texcoord.x / texture.TEX_WIDTH;
	float texright = (texcoord.x + texcoord.w) / texture.TEX_WIDTH;
	float textop = texcoord.y / texture.TEX_HEIGHT;
	float texbottom = (texcoord.y + texcoord.h) / texture.TEX_HEIGHT;
	...
	if(rotationAngle == 0) {
		...
		double x0 = pos.x + 0.5*pos.w, y0 = pos.y + 0.5*pos.h;
		pair<double,double> p0 = rotatePt(x0, y0,
			pos.x, pos.y, rotationAngle);
		pair<double,double> p1 = rotatePt(x0, y0,
			pos.x, pos.y+pos.h, rotationAngle);
		pair<double,double> p2 = rotatePt(x0, y0,
			pos.x+pos.w, pos.y+pos.h, rotationAngle);
		pair<double,double> p3 = rotatePt(x0, y0,
			pos.x+pos.w, pos.y, rotationAngle);
...
void drawTexture(int texNum, Rect pos, double z, double a, double rotationAngle) {
	Texture& texture = globalTexture[texNum];

	drawTexture(texNum, pos,
		Rect(0.0f, 0.0f, texture.w, texture.h),
		z, a, rotationAngle);
}

void drawTile(int texNum, Rect pos, Rect texcoord, TileOrient orient,
	...
	case TILE_ORIENT_FLIP_V:
		textop += texbottom;
		texbottom = textop - texbottom;
		textop = textop - texbottom;
		break;
	case TILE_ORIENT_FLIP_H:
	case TILE_ORIENT_FH_90:
	case TILE_ORIENT_FH_270:
		texleft += texright;
		texright = texleft - texright;
		texleft = texleft - texright;
		break;
	default:
		break;
	}

	float offsetx = texleft + texright / 2.0f;
	float offsety = textop + texbottom / 2.0f;
...
int loadTexture(const char *filename, int* width, int* height)
{
	...
	if ( iter == textureFileLookup.end() ) {
		...
		Texture& t = globalTexture[currNum];
		if(width) {
			*width = t.w;
		}
